        ...
        for SV in SVs:
            ...
            if ME_type == "SVA" or ratio >= 0.8:
                for rec in target_VCF.fetch(
                    ...
                    # match on ref, alt (from fasta), svlen,
                    # and svtype==insertion
                    # SVLEN is one less because alt length includes the ref nuc
                    if (
                        rec.ref == SV["ref"]
                        and svlen == SV["length"] - 1
                        and rec.info["SVTYPE"] == "INS"
                        and rec.alts[0] == str(INS_fa[SV["id"]].seq)
                    ):
                        ...
                        #                        shared_samples = list(
                        #                            set(SV_samples) & set(MEI_samples)
                        #                        )
                        #                        if shared_samples:
                        #                            # at least one shared sample
                        #                            # between MEI and SV calls
