...
def main():
    if len(sys.argv)!=6:
        sys.stderr.write("Usage: program.py PALMER_calls PALMER_read_seqs type sample ref_fai\n")
        sys.exit(1)
    ...
    for ID in seqs_all:
        ...
        seq[ID] = seqs_all[ID][int(len(seqs_all[ID])/2)]  # this retrieves the median value from the sorted list
    ...
    for line in callfile:
        ...
        line = line.strip().split('\t')
        cur = {}
        cid, cur["chrom"], cur["start"], cur["conf"], cur["ori"], cur["polyAlen"], cur["5TSDlen"], cur["3TSDlen"], cur["transDlen"] = line[0], line[1], line[2], line[10], line[13], line[14], line[15], line[16], line[17]
    ...
    for cid in calls:
        cur = calls[cid]
        print('%s\t%s\t%s\tN\t%s\t60\t.\tSVTYPE=INS;ME_TYPE=%s;SVLEN=%d;CONF_READS=%s;ORI=%s;POLYA_LEN=%s;TSD_5PRIME_LEN=%s;TSD_3PRIME_LEN=%s;TRANSD_LEN=%s\tGT\t0/1' % (cur["chrom"], cur["start"], cid, cur["seq"], MEtype, len(cur["seq"]), cur["conf"], cur["ori"], cur["polyAlen"], cur["5TSDlen"], cur["3TSDlen"], cur["transDlen"]))
