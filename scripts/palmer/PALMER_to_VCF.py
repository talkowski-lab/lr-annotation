...
def main():
    ...
    for line in callfile:
        ...
        # there are multiple possible positions given, so take the median (usually all four are the same but not always)
        cur["pos"] = round(numpy.median([int(line[2]), int(line[3]), int(line[4]), int(line[5])]))

        # calculate length of MEI
        tmp_end = round(numpy.median([int(line[8]), int(line[9])]))
        tmp_start = round(numpy.median([int(line[6]), int(line[7])]))

        # if there is a 5' inversion (common in LINEs), the length calculation is different:
        if cur['5inv']:
            tmp_5invend = int(line[19])
            tmp_5invstart = int(line[20])
            cur["length"] = tmp_end - tmp_start + int(cur["transDlen"]) + tmp_5invend - tmp_5invstart

        else:  # length without 5' inversion: end-start of consensus positions + transD length (note this is NOT RELIABLE FOR SVAs)
