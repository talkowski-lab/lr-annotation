...
def main():
    ...
    print(
        "##INFO=<ID=INVERSION_5PRIME,Number=0,Type=Flag,"
        'Description="Whether the MEI has a 5 prime inversion">'
    )
    ...
    for line in callfile:
        ...
        cur["pos"] = round(
            numpy.median([int(line[2]), int(line[3]), int(line[4]), int(line[5])])
        )
