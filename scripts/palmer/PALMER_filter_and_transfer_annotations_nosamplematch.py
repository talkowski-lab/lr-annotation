...
# intersection output:
# 0:chrom 1:range_start 2:range_end 3:position(s) 4:REF(s) 5:SVLEN(s) (<< from target VCF <<) 6:chrom 7:pos-1 8:pos 9:mei_len 10:sample(s) (<< from MEI VCF <<)

def main():
    if len(sys.argv) != 5:
        sys.stderr.write("Usage: program.py intersect_output target_VCF INS_fa ME_type\n")
        sys.exit(1)
    ...
    for line in intersect_output:
        ...
        for ref, pos, length in zip(SV_refs, SV_positions, SV_lengths):
            SVs.append({'ref':ref.split('_')[0], 'pos':int(pos), 'length':int(length), 'id':"%s:%s;%s" % (chrom, pos, ref)})
        ...
        for SV in SVs:
            ...
            if ME_type == "SVA" or ratio >= 0.8:
                for rec in target_VCF.fetch(chrom, SV['pos']-1, SV['pos']): # tested that this is the right way to fetch one position
                    ...
                    # match on ref, alt (from fasta, just to be 100% sure we have the right record), svlen, and svtype==insertion
                    # SVLEN is one less because alt length includes the ref nuc
                    if rec.ref == SV['ref'] and svlen == SV['length']-1 and rec.info['SVTYPE']=="INS" and rec.alts[0]==str(INS_fa[SV['id']].seq):
...
#                        if shared_samples: # there has to be at least one shared sample between the MEI and SV calls (should we be more stringent?)
                            # write annotation file:
                        sys.stdout.write("%s\t%s\t%s\t%s\t%s\t%d\n" % (chrom, SV['pos'], rec.ref, rec.alts[0], ME_type, MEI_length))
 #                       else:
