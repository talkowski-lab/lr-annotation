...
def get_vep_annotations(info_dict, vep_key: str, indices: Dict[int, str]) -> Dict[str, str]:
    vep_string = ''
...
def write_summary_stats(final_vcf_path: str, contig: str, output_path: str):
    ...
    for match_type, count in match_counts.items():
        if match_type != 'UNMATCHED':
            ...
            summary_stats[f'{str(match_type).lower()}_percent'] = (count / total_variants * 100) if total_variants > 0 else 0.0
    with open(output_path, 'w') as f:
        f.write('\t'.join(summary_stats.keys()) + '\n')
        f.write('\t'.join(str(v) for v in summary_stats.values()) + '\n')


def write_summary_table(final_vcf_path: str, truth_variants: Dict[str, Dict[str, str]], vep_keys: Tuple[str, str, Dict[int, str], Dict[int, str]], output_path: str):
    all_rows = []
        ...
        for record in vcf_in:
            ...
            if 'gnomAD_V4_match_ID' in record.info:
                ...
                if match_id in truth_variants:
                    ...
                    truth_af_pairs_raw = {k: v for k, v in truth_info.items() if is_af_field(k)}
                    truth_af_pairs = {}
                    ...
                    for category in set(eval_indices.values()) | set(truth_indices.values()):
...
def main():
    ...
    eval_indices = {i: cat for i, cat in enumerate(eval_fields) if cat in common_categories}
    truth_indices = {i: cat for i, cat in enumerate(truth_fields) if cat in common_categories}

    summary_table_path = f"{args.prefix}.benchmark_summary.tsv"
    write_summary_table(args.final_vcf, truth_info, (eval_vep_key, truth_vep_key, eval_indices, truth_indices), summary_table_path)
