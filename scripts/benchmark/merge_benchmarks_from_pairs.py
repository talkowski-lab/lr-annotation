...
def main():
    ...
    log_memory(
        f"Found {len(af_pair_paths)} AF pair files and "
        f"{len(vep_pair_paths)} VEP pair files"
    )
    ...
    for i, p in enumerate(af_pair_paths):
        log_memory(
            f"Processing AF file {i+1}/{len(af_pair_paths)}: {os.path.basename(p)}"
        )
        file_size_mb = os.path.getsize(p) / (1024 * 1024)
        log_memory(f"AF file size: {file_size_mb:.2f} MB")

        with gzip.open(p, "rt") as fh:
            log_memory("Starting to read AF file in chunks")
            chunk_count = 0
            ...
            for chunk in reader:
                ...
                if "af_key" not in chunk.columns:
                    log_memory(
                        f"Warning: 'af_key' not in columns: {chunk.columns.tolist()}"
                    )
                    continue
                ...
                for key, sub in chunk.groupby("af_key"):
                    ...
                    if chunk_count % 10 == 0:
                        largest_keys = sorted(
                            [(k, len(v)) for k, v in af_groups.items()],
                            key=lambda x: x[1],
                            reverse=True,
                        )[:3]
    ...
    for i, p in enumerate(vep_pair_paths):
        log_memory(
            f"Processing VEP file {i+1}/{len(vep_pair_paths)}: {os.path.basename(p)}"
        )
        file_size_mb = os.path.getsize(p) / (1024 * 1024)
        log_memory(f"VEP file size: {file_size_mb:.2f} MB")

        with gzip.open(p, "rt") as fh:
            log_memory("Starting to read VEP file in chunks")
            chunk_count = 0
            reader = pd.read_csv(
            ...
            for chunk in reader:
                ...
                for cat, sub in chunk.groupby("category"):
                    ...
                    if chunk_count % 10 == 0:
                        largest_cats = sorted(
                            [(k, len(v)) for k, v in vep_lists.items()],
                            key=lambda x: x[1],
                            reverse=True,
                        )[:3]
    ...
    for cat, frames in vep_lists.items():
        ...
        log_memory(
            f"Groupby complete for VEP category '{cat}', df size: {len(df_grouped)}"
        )
