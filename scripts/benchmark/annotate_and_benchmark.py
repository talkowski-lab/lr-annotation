...
def main():
    ...
    parser.add_argument("--log_scale", action="store_true", help="Use log scale for AF plots.")
        ...
        for record in vcf_in:
            ...
            if 'gnomAD_V4_match_ID' in record.info:
                ...
                if match_id in truth_variants:
                    matched_data.append({
                        'eval': record.info, 
                        'truth': truth_variants[match_id]}
                    )
                else:
                    print(f"Matching variant {match_id} for {record.id} not found in truth VCF")
    ...
    eval_indices = {i: cat for i, cat in enumerate(eval_vep_categories) if cat in common_categories}
    truth_indices = {i: cat for i, cat in enumerate(truth_vep_categories) if cat in common_categories}
    ...
    for item in matched_data:
        # AF data
        eval_af_pairs = {normalize_af_field(k): normalize_af_value(v) for k, v in item['eval'].items() if is_af_field(k)}
        truth_af_pairs = {normalize_af_field(k): normalize_af_value(v) for k, v in item['truth'].items() if is_af_field(k)}
        for eval_key, eval_af in eval_af_pairs.items():
            if eval_key in truth_af_pairs:
                if eval_af > 0 and truth_af_pairs[eval_key] > 0:
                    af_data[eval_key].append({'eval_af': eval_af, 'truth_af': truth_af_pairs[eval_key]})
        ...
        for category in common_categories:
            if category in eval_annos or category in truth_annos:
                vep_data_by_category[category]['eval'].append(eval_annos.get(category, 'N/A'))
                vep_data_by_category[category]['truth'].append(truth_annos.get(category, 'N/A'))
    
    # AF plots
    for af_key, af_data in af_data.items():
        af_key_str = '_'.join(sorted(list(af_key)))
        plot_af_correlation(pd.DataFrame(af_data), af_key_str, af_plot_dir, args.log_scale)
