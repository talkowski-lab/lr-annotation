...
def vcf_to_df(vcf_path: str) -> pl.DataFrame:    
    ...
    with pysam.VariantFile(vcf_path) as vcf_in:
        ...
        for record in vcf_in:
            ...
            svlen_val = record.info.get("SVLEN") if "SVLEN" in header_info_keys else None
            ...
            varsubtype = record.info.get("VARSUBTYPE") if "VARSUBTYPE" in header_info_keys else None
            var_score = record.info.get("SCORE") if "SCORE" in header_info_keys else None
            ...
            var_data: Dict[str, Any] = {
                ...
                "seq": record.alts[0] if record.alts and not record.alts[0].startswith("<") else None
            }
...
def df_to_vcf(df: pl.DataFrame, template_vcf_path: str, output_vcf_path: str) -> None:
    ...
    with pysam.VariantFile(template_vcf_path) as vcf_in:
        ...
        if "VARTYPE" not in [h.name for h in header.info.values()]:
            header.add_line('##INFO=<ID=VARTYPE,Number=1,Type=String,Description="Variant type">')
        if "VARSUBTYPE" not in [h.name for h in header.info.values()]:
            header.add_line('##INFO=<ID=VARSUBTYPE,Number=1,Type=String,Description="Variant subtype">')
        ...
        if "SCORE" not in [h.name for h in header.info.values()]:
            header.add_line('##INFO=<ID=SCORE,Number=1,Type=Float,Description="Variant score">')
    ...
    with pysam.VariantFile(output_vcf_path, "w", header=header) as vcf_out:
        for row in df_sorted:
            ...
            if not alt_allele or (isinstance(alt_allele, str) and alt_allele == vartype):
