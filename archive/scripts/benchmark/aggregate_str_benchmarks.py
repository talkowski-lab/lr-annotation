...
def plot_similarity_across_callers(output_path, df):
    ...
    ax.hist(
        [divergent, similar, exact],
        bins=bins,
        stacked=True,
        label=[
            f"< 90% Similar ({div_pct:.1f}%)",
...
def plot_length_difference_across_callers(output_path, df):
    ...
    for col, (diffs, label) in enumerate(
        [(all_diffs, "All Motifs")]
...
def plot_edit_distance_to_reference(output_path, df):
    ...
    for col, (dists, label) in enumerate(
        [(trgt_all, "All Motifs")]
        + [(trgt_by_bin[i], size_ranges[i][2]) for i in range(3)]
    ):
        ax = axes[0, col]
...
def generate_outputs(output_prefix, df, suffix, include_ref_plots):
    ...
    write_concordance_matrix(
        f"{output_prefix}.{suffix}.genotype_concordance_matrix.tsv", df
    )
    plot_similarity_across_callers(
        f"{output_prefix}.{suffix}.similarity_across_callers.png", df
    )
    plot_edit_distance_across_callers(
        f"{output_prefix}.{suffix}.edit_distance_across_callers.png", df
    )
    plot_length_difference_across_callers(
        f"{output_prefix}.{suffix}.length_difference_across_callers.png", df
    )
    plot_length_difference_vs_locus_size(
        f"{output_prefix}.{suffix}.length_difference_vs_locus_size.png", df
    )

    if include_ref_plots:
        plot_edit_distance_to_reference(
            f"{output_prefix}.{suffix}.edit_distance_to_reference.png", df
        )
        plot_length_difference_to_reference(
            f"{output_prefix}.{suffix}.length_difference_to_reference.png", df
        )


def main():
    parser = argparse.ArgumentParser(
        description="Aggregate STR benchmark statistics and generate plots"
    )
    parser.add_argument(
        "--stats-files",
        required=True,
        help="File containing list of per-sample stats TSVs",
    )
    parser.add_argument(
        "--output-prefix", required=True, help="Output prefix for all files"
    )
    parser.add_argument(
        "--include-all-regions",
        required=True,
    ...
    if include_all:
        generate_outputs(
            args.output_prefix, df_all, "all_regions", include_ref_plots=True
        )

    df_non_ref = df_all[
        (df_all["trgt_non_ref_count"] > 0) | (df_all["vamos_non_ref_count"] > 0)
    ]
    generate_outputs(
        args.output_prefix, df_non_ref, "non_ref_regions", include_ref_plots=False
    )
    ...
    print(
        f"Non-ref regions: {len(df_non_ref)} ({len(df_non_ref)/len(df_all)*100:.1f}%)"
    )
