...
def write_vcf_header(f, sample_id: str, ref_fa: Path, parquet_files: list):
    ...
    f.write("##source=PAV3_parquet_to_vcf\n")
...
def main():
    ...
    parser.add_argument("--sample", required=True, help="Sample ID for VCF")
    parser.add_argument(
    ...
    combined_df = combined_df.sort("SAMPLE", descending=True, nulls_last=True).unique(
