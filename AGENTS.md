...
data/                # Not synced to the remote repo (gitignored) - out of scope for these instructions
...
flake8 scripts/
```
The `.flake8` config at repo root sets `max-line-length = 130`, extends ignore for `E203` and `W503`, and excludes `data/*` and `archive/*`.
...
## Python Conventions
- Lint with `flake8`; config lives in `.flake8` at repo root (`max-line-length = 130`, ignores `E203`/`W503`).
...
1. **Scripts** (`scripts/`): Implement core logic if needed - prefer to use in-line Python within workflows though, unless explicitly asked to create a separate script.
...
5. **Docs** (`docs/`): Document new references, output schema fields, or pipeline steps.
...
- `docs/` — Extended documentation: conventions, pipeline narrative, per-workflow inputs/outputs, repository structure.
...
- Do not document anything under `data/` in `docs/` — it is gitignored and not synced to the remote repo.
