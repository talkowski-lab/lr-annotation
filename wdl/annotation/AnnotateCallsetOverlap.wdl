...
import "../utils/ExactMatchShard.wdl"
    ...
    scatter (contig in contigs) {
        ...
        if (defined(shard_bin_size_exact_match)) {
            ...
            scatter (k in range(length(CreateExactShards.shard_regions))) {
                call ExactMatchShard.ExactMatchShard {
                    input:
                        vcf = vcf_subsetted,
            ...
            call Helpers.ConcatTsvs as ConcatExactAnnotations {
                input:
                    tsvs = ExactMatchShard.annotated_tsv,
                    sort_output = true,
            ...
            call Helpers.ConcatVcfs as ConcatTruvariEval {
                input:
                    vcfs = ExactMatchShard.truvari_eval_vcf,
                    vcf_idxs = ExactMatchShard.truvari_eval_vcf_idx,
            ...
            call Helpers.ConcatVcfs as ConcatTruvariTruth {
                input:
                    vcfs = ExactMatchShard.truvari_truth_vcf,
                    vcf_idxs = ExactMatchShard.truvari_truth_vcf_idx,
                    allow_overlaps = false,
                    naive = false,
