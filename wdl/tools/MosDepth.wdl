...
workflow AlignedMetrics {
    ...
    if (defined(gcs_output_dir)) {
        ...
        call Helpers.FinalizeToDir as FFCoverageFullDist { input: outdir = outdir + "/coverage/", files = MosDepth.full_dist }
        call Helpers.FinalizeToDir as FFCoverageGlobalDist { input: outdir = outdir + "/coverage/", files = MosDepth.global_dist }
        call Helpers.FinalizeToDir as FFCoverageRegionDist { input: outdir = outdir + "/coverage/", files = MosDepth.region_dist }
        call Helpers.FinalizeToDir as FFCoverageRegions { input: outdir = outdir + "/coverage/", files = MosDepth.regions }
        call Helpers.FinalizeToDir as FFCoverageRegionsCsi { input: outdir = outdir + "/coverage/", files = MosDepth.regions_csi }
        call Helpers.FinalizeToDir as FFCoverageQuantizedDist { input: outdir = outdir + "/coverage/", files = MosDepth.quantized_dist }
        call Helpers.FinalizeToDir as FFCoverageQuantized { input: outdir = outdir + "/coverage/", files = MosDepth.quantized }
        call Helpers.FinalizeToDir as FFCoverageQuantizedCsi { input: outdir = outdir + "/coverage/", files = MosDepth.quantized_csi }

        call Helpers.FinalizeToDir as FFDepthSummaries { input: outdir = outdir + "/coverage_summaries/", files = SummarizeDepth.cov_summary }
    }
...
task CoverageTrack {
    ...
    command <<<
        ...
        samtools depth \
            -a ~{bam} \
            -r ~{chr}:~{start}-~{end} \
        | bgzip > ~{basename}.coverage.~{chr}_~{start}_~{end}.txt.gz
    ...
    RuntimeAttr default_attr = object {
        cpu_cores: 1,
        mem_gb: 4,
        disk_gb: 2 * ceil(size(bam, "GB") + size(bai, "GB")),
        boot_disk_gb: 10,
        preemptible_tries: 2,
        max_retries: 0
    }
...
task SamStats {
    ...
    RuntimeAttr default_attr = object {
        cpu_cores: 1,
        mem_gb: 4,
        disk_gb: Int disk_size = 2 * ceil(size(bam, "GB")),
        boot_disk_gb: 10,
        preemptible_tries: 2,
        max_retries: 0
    }
...
task ComputeBedCoverage {
    ...
    command <<<
        ...
        zcat ~{prefix}.txt.gz \
            | awk '{ sum += sprintf("%f", $15*$16) } END { printf("%f\n", sum) }' \
            > ~{prefix}.count.txt
    >>>
    ...
    RuntimeAttr default_attr = object {
        cpu_cores: 1,
        mem_gb: 2,
        disk_gb: 2 * ceil(size(bam, "GB") + size(bai, "GB") + size(bed, "GB")),
        boot_disk_gb: 10,
        preemptible_tries: 2,
        max_retries: 0
    }
...
task BamToBed {
    ...
    RuntimeAttr default_attr = object {
        cpu_cores: 2,
        mem_gb: 8,
        disk_gb: 4 * ceil(size(bam, "GB") + size(bai, "GB")),
        boot_disk_gb: 10,
        preemptible_tries: 2,
        max_retries: 0
    }
