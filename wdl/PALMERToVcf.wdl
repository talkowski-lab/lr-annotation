...
workflow PALMERToVcf {
    input {
        ...
        String palmer_to_vcf_docker
        RuntimeAttr? runtime_attr_override_palmer_to_vcf
    }

    call call_PALMER_to_VCF { 
        input: 
            PALMER_calls=PALMER_calls, 
            ref_fai=ref_fai, 
            sample=sample, 
            ME_type = ME_type,
            docker = palmer_to_vcf_docker,
            runtime_attr_override = runtime_attr_override_palmer_to_vcf
    }

    output {
        File PALMER_vcf = call_PALMER_to_VCF.vcf
        File PALMER_vcf_tbi = call_PALMER_to_VCF.vcf_tbi
    }
}

task call_PALMER_to_VCF {
    input {
        File PALMER_calls
        String ME_type
        String sample
